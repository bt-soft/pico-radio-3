...
class Button : public UIComponent {
  ...
  public:
    Button(TFT_eSPI &tft, const Rect &bounds, const String &text, const ColorScheme &colors = ColorScheme::defaultScheme())
        : UIComponent(tft, bounds, colors), text(text) {} // Szöveg beállítása
    ...
    virtual void draw() override {
        ...
        if (!enabled) {
            ...
            bgColor = colors.pressedBackground;
            fgColor = colors.pressedForeground;
        ...
        // Keret rajzolása
        tft.drawRect(bounds.x, bounds.y, bounds.width, bounds.height, borderColor); // Szöveg rajzolása
        if (!text.isEmpty()) {
            tft.setTextSize(textSize);
            tft.setTextColor(fgColor);
            tft.setTextDatum(MC_DATUM); // Middle Center
            tft.drawString(text, bounds.centerX(), bounds.centerY());
        }

        needsRedraw = false; // Újrarajzolás kész
    }
    virtual bool handleTouch(const TouchEvent &event) override { return UIComponent::handleTouch(event); }

  protected:
    virtual void onClick(const TouchEvent &event) override {
        DEBUG("Button clicked: '%s' at (%d,%d)\n", text.c_str(), event.x, event.y);
        if (clickCallback) {
            DEBUG("Executing button callback for '%s'\n", text.c_str());
            clickCallback();
        } else {
            DEBUG("No callback set for button '%s'\n", text.c_str());
        }
    }
};
