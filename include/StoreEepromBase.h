...
#ifndef EEPROM_SIZE
#define EEPROM_SIZE \
  2048  // Alapértelmezett 2K méret (512-4096 között módosítható)
#endif
...
class StoreEepromBase {
 public:
   ...
   */
  static uint16_t load(T& data, uint16_t address = 0,
                       const char* className = "Ismeretlen") {
    bool valid = false;
    uint16_t crc = getIfValid(data, valid, address, className);

    if (!valid) {
      DEBUG(
          "[%s] EEPROM tartalom érvénytelen a %d címen, alapértékek mentése!\n",
          className, address);
   ...
   */
  static uint16_t save(const T& data, uint16_t address = 0,
                       const char* className = "Ismeretlen") {
    uint16_t crc =
        Utils::calcCRC16(reinterpret_cast<const uint8_t*>(&data), sizeof(T));

    DEBUG("[%s] Adatok mentése EEPROM %d címre (%d bájt)...", className,
          address, sizeof(T));
   ...
   * @return CRC16 ellenőrző összeg
   */
  static uint16_t getIfValid(T& data, bool& valid, uint16_t address = 0,
                             const char* className = "Ismeretlen") {
    T tempData;
    uint16_t storedCrc;
    ...
    // CRC ellenőrzés
    uint16_t calculatedCrc = Utils::calcCRC16(
        reinterpret_cast<const uint8_t*>(&tempData), sizeof(T));
    valid = (storedCrc == calculatedCrc);

    DEBUG(
        "[%s] EEPROM ellenőrzés %d címen. Tárolt CRC: %d, Számított CRC: %d -> "
        "%s\n",
        className, address, storedCrc, calculatedCrc,
        valid ? "Érvényes" : "ÉRVÉNYTELEN");
   ...
   */
  static constexpr size_t getRequiredSize() {
