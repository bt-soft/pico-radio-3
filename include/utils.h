 ...
 */
template <typename T>
void mergeArrays(const T *source1, uint8_t length1, const T *source2,
                 uint8_t length2, T *destination, uint8_t &destinationLength) {
 ...
 */
template <typename T>
static uint16_t calcCRC16(const T &obj) {
