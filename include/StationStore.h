...
#include "BaseStationStore.h"    // Közös ősosztály
#include "DebugDataInspector.h"  // Debug kiíratáshoz
...
// --- FM Station Store ---
class FmStationStore
    : public BaseStationStore<FmStationList_t, MAX_FM_STATIONS> {
 protected:
  ...
  uint16_t performSave() override {
    uint16_t savedCrc = StoreEepromBase<FmStationList_t>::save(
        getData(), EEPROM_FM_STATIONS_ADDR, getClassName());
#ifdef __DEBUG
    if (savedCrc != 0) DebugDataInspector::printFmStationData(getData());
#endif
    return savedCrc;
  }

  uint16_t performLoad() override {
    uint16_t loadedCrc = StoreEepromBase<FmStationList_t>::load(
        getData(), EEPROM_FM_STATIONS_ADDR, getClassName());
#ifdef __DEBUG
 ...
 public:
  FmStationStore() : BaseStationStore<FmStationList_t, MAX_FM_STATIONS>() {
    data = DEFAULT_FM_STATIONS;
  }
...
// --- AM Station Store ---
class AmStationStore
    : public BaseStationStore<AmStationList_t, MAX_AM_STATIONS> {
 protected:
  const char* getClassName() const override { return "AmStationStore"; }

  // Felülírjuk a mentést/betöltést a helyes címmel és névvel
  uint16_t performSave() override {
    uint16_t savedCrc = StoreEepromBase<AmStationList_t>::save(
        getData(), EEPROM_AM_STATIONS_ADDR, getClassName());
#ifdef __DEBUG
    if (savedCrc != 0) DebugDataInspector::printAmStationData(getData());
#endif
    return savedCrc;
  }

  uint16_t performLoad() override {
    uint16_t loadedCrc = StoreEepromBase<AmStationList_t>::load(
        getData(), EEPROM_AM_STATIONS_ADDR, getClassName());
#ifdef __DEBUG
 ...
 public:
  AmStationStore() : BaseStationStore<AmStationList_t, MAX_AM_STATIONS>() {
    data = DEFAULT_AM_STATIONS;
  }
