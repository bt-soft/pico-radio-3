...
#include "defines.h"  // Szükséges a CW_DECODER_DEFAULT_FREQUENCY makróhoz
...
const Config_t DEFAULT_CONFIG = {
    ...
    // BandWidht
    .bwIdxAM =
        0,  // BandWidth AM - Band::bandWidthAM index szerint -> "6.0" kHz
    .bwIdxFM = 0,  // BandWidth FM - Band::bandWidthFM[] index szerint -> "AUTO"
    .bwIdxMW =
        0,  // BandWidth MW - Band::bandWidthAM index szerint -> "6.0" kHz
    ...
    // AGC
    .agcGain =
        static_cast<uint8_t>(Si4735Utils::AgcGainMode::Automatic),  // -> 1,
    .currentAGCgain =
        static_cast<uint8_t>(Si4735Utils::AgcGainMode::Automatic),  // -> 1
    ...
    .tftBackgroundBrightness =
        TFT_BACKGROUND_LED_MAX_BRIGHTNESS,  // TFT Háttérvilágítás
    .tftDigitLigth = true,                  // Inaktív szegmens látszódjon?
    .screenSaverTimeoutMinutes =
        SCREEN_SAVER_TIMEOUT,  // Képernyővédő alapértelmezetten 5 perc
    ...
    .miniAudioFftConfigAnalyzer =
        0.0f,                        // Analyzerhez alapértelmezetten Auto Gain
    .miniAudioFftConfigRtty = 0.0f,  // RTTY-hez alapértelmezetten Auto Gain

    // CW vételi eltolás
    .cwReceiverOffsetHz =
        CW_DECODER_DEFAULT_FREQUENCY,  // Default CW vételi eltolás Hz-ben
};
